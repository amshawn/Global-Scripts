...
#set rebate recipient-----------------------------------------------------------
		if Quote.GetCustomField('MG_H_AGREEMENTTYPE').Content == "End Customer":
			Quote.CustomFields.AssignValue('BO_CF_END_CUSTOMER', "MBO" + row.Cells["REBATE_RECIPIENT"].Value.split(",")[0])
			if row["SAP_SENT_ON"]:
				#set rebate recipient as readonly
				Quote.GetCustomField('BO_CF_END_CUSTOMER').Editable	  = False
			...
			if row["SAP_SENT_ON"]:
				#set rebate recipient as readonly
				Quote.GetCustomField('BO_CF_REBATE_RECIPIENT').Editable	  = False
# set the values in the table for custom fields---------------------------------
		...
		if row["SAP_SENT_ON"]:
			Quote.GetCustomField('BO_CF_PAY_CURRENCY').Editable	  = False
#get rebate scale---------------------------------------------------------------
