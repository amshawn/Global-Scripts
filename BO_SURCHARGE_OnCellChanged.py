...
try:
	if Param is not None:
		...
		#1513 get changed row index
		argsIdx = EventArgs.ChangedCell.RowIndex
		#1513 get changed container
		argsContainer = EventArgs.Container
		if EventArgs.ChangedCell.ColumnName == "BO_AMOUNT":
#1513 START---------------------------------------------------------------------
			if argsContainer.Rows[argsIdx].Columns["BO_SCALE_MIN"].Value:
				#get surcharge code
				code = argsContainer.Rows[argsIdx].Columns["BO_CODE"].Value
				#look for rows with same code
				for row in argsContainer.Rows:
					if row["BO_CODE"] == code:
						#set flag code updated
						row["IS_UPDATED"] = "True"
					else:
						#set flag code updated
				...
				#set flag code Updated
				argsContainer.Rows[argsIdx].Columns["IS_UPDATED"].Value = "True"
#1513 END---------------------------------------------*-------------------------
			#Get container
